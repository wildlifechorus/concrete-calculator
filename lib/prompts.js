...
export async function promptLightweightOptions(presetCementWeight = null) {
  let cementWeight = presetCementWeight;

  if (cementWeight === null) {
    const { amount } = await inquirer.prompt([
      {
        type: 'number',
        name: 'amount',
        message: 'How much cement do you want to use? (in grams)',
        validate: (input) => input > 0 || 'Please enter a positive number',
      },
    ]);
    cementWeight = amount;
  ...
  return { cementWeight, cementType, useCSA, useX33, useFibers, pigmentPercent, pigmentKey };
}

export async function promptLightweightMoldVolume() {
  const inputs = await inquirer.prompt([
    {
      type: 'number',
      name: 'volume',
