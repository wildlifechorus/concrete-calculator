...
- See the full 0–10% pigment range strip with the **industry saturation point marked at 7%**
- Get **per-dose guidance** (washed-out / optimum / saturated / wasted) calibrated against Solomon Colors' published loading chart
...
### How the preview is calculated

The simulator uses a physically-based color model rather than naive RGB blending:

- **Pigment ↔ cement mixing** uses [Spectral.js](https://github.com/rvanwijnen/spectral.js)
  (Kubelka–Munk theory), the same model paint chemists use for color matching. Bundled
  locally in `lib/vendor/spectral.min.js` (MIT-licensed, ~12 KB, no network required).
- **Saturation curve**: `f(pct) = 1 − exp(−0.50 · pct)`. Color develops rapidly through
  4%, plateaus by 6–7%, and is essentially flat above 8%. Calibrated against Solomon
  Colors' published 7% saturation point, adding more pigment beyond 7% wastes material
  without darkening the color.
- **Sand visibility** in a 1:1 cement:sand mortar is set to 30% for a smooth mold-cast
  surface. Pigment colors only the cement paste; sand grains dilute the visible color.
- **Wet vs dry shift**: fresh / wet concrete is rendered ~32% darker (multiplier 0.68),
  matching the "hose-it-down to preview sealed color" guidance from Solomon Colors.

Open the **"How accurate is this preview?"** panel in the simulator for the full list of
what is and isn't modeled, plus the recommendation to always pour a small test puck for
high-stakes pieces.
...
│   └── vendor/
│       └── spectral.min.js   # Spectral.js Kubelka–Munk pigment mixing (MIT, bundled)
├── scripts/
